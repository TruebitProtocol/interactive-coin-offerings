...
library InteractiveCrowdsaleLib {
  ...
  function submitBid(InteractiveCrowdsaleStorage storage self,
    ...
    if(now >= endWithdrawalTime){
      ...
      // if the personal minimum is less than or equal to the current valuation
      // the bid goes in.
      if(_personalMinimum <= self.valuationPointer){
        ...
        // else if the personal minimum is greater than the current value, we
        // need to waterfall down from the top to see if we can now meet the
        // personal minimum declared
        while(_currentBucket >= _personalMinimum){
          _proposedCommit += self.valueMinAndCap[_currentBucket][1];

          // If the amount committed has exceeded a bucket during this loop
          // then we've increased value
          if(_proposedCommit < _currentBucket){
            _proposedCommit -= self.valueMinAndCap[_currentBucket][0];
          } else {
            self.valuationPointer = _currentBucket;
            self.valueCommitted = _proposedCommit;
            break;
