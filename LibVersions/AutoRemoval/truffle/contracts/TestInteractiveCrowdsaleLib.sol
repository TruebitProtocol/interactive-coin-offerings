...
library TestInteractiveCrowdsaleLib {
  ...
  function submitBid(InteractiveCrowdsaleStorage storage self, uint256 _amount, uint256 _personalValuation, uint256 _listPredict, uint256 _currtime) returns (bool) {
    ...
    if (_currtime < self.endWithdrawlTime) {
    ...
    LogBidAccepted(msg.sender, self.base.hasContributed[msg.sender], _personalValuation);
  ...
  function withdrawBid(InteractiveCrowdsaleStorage storage self, uint256 _currtime) returns (bool) {
    ...
    uint256 multiplierPercent = (100*((self.endWithdrawlTime + self.base.milestoneTimes[0]) - _currtime))/self.endWithdrawlTime;
